...
export const actionGetCreateLeadDataStart = data => ({
  type: ACTION_GET_CREATE_LEAD_DATA_START,
  data
});

export const actionGetCreateLeadDataSuccess = data => ({
  type: ACTION_GET_CREATE_LEAD_DATA_SUCCESS,
  data
});
export const actionGetCreateLeadSuccess = data => ({
  type: ACTION_CREATE_LEAD_SUBMIT_SUCCESS,
  data
});
  ...
  return async dispatch => {
    ...
    if (!response.isError) {
      LocalStorageHelper.add(
        LocalStorageConfig.KEY_LEAD_FORM_DATA,
        response.data
      );
...
export const actionCreateLead = state => {
  return async dispatch => {
    ...
    if (!response.isError) {
      ...
      dispatch(actionGetCreateLeadDataStart(response));
      return true;
    }
    dispatch(actionSetErrorDialogDisplayState(response.errorMessage));
  };
